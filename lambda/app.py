...
def compute_coffee_dates(Groups, number_dates: int):
    """
    Groups is a 2D nested list containing the groups that are to be matched
    """
    persons = []
    dates = []
    G = nx.Graph()
...
def handler(event, context):
    ...
    groups = decodedEvent["groups"]
    number_dates = decodedEvent["number_dates"]
    return {
        "statusCode": 200,
